...
export default function TopBar() {
  ...
  const getAvatarColor = (name: string) => {
    const colors = [
      ...
      "bg-[#312e81] text-sky border-[#312e81]", // Indigo
      "bg-[#451a03] text-[var(--earth)] border-[#451a03]", // Amber
  ...
  return (
    <div className="flex items-center justify-between py-3 px-6 lg:px-12 border-b border-border-s bg-surface sticky top-0 z-[100]">
      <div className="font-serif text-2xl font-semibold text-txt flex items-center gap-2"></div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 pl-4 border-l border-border-s">
          <button className="group relative flex items-center justify-center">
            <div
              className={`w-8 h-8 rounded shrink-0 flex items-center justify-center font-sans text-sm font-bold tracking-tighter cursor-pointer transition-colors border border-border-s ${getAvatarColor(
                displayName,
              )}`}
            >
              {initials}
